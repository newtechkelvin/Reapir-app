      ...
      vehicles.forEach((vehicle: any) => {
        ...
        if (Array.isArray(orders)) {
          orders.forEach((wo: any) => {
            ...
            // 只要狀態不是 completed / closed / 已完成，皆視為進行中的 Open 工單
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center bg-slate-800 text-white p-4 rounded-xl shadow-sm gap-2">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">目前全廠進行中 (Open) 的工單共有 <span className="font-bold text-amber-400 text-sm">{openOrders.length}</span> 張（按開單時間最久的優先排序）</p>
        </div>
        <button
          type="button"
      ...
      {isLoading ? (
        <div className="text-center py-12 text-gray-500 font-semibold animate-pulse">⏳ 正在向資料庫讀取所有進行中 (Open) 的工單...</div>
      ) : openOrders.length === 0 ? (

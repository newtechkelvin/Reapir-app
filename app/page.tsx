        ...
        {activeTab === 'create' && (
          <form onSubmit={handleCreateOrder} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">車型 / 品牌</label>
                <input
                  type="text"
            ...
            <div>
              ...
              {items.map((item, idx) => (
                <div key={idx} className="flex flex-wrap md:flex-nowrap gap-2 mb-2 items-center border-b pb-2">
                  ...
                  >
                    <option value="Labor">人工 / 工時</option>
                    <option value="Part">零件 / 耗材</option>
                  </select>
                  <input
                    type="text"
                    placeholder="項目名稱 (如: 更換機油)"
          ...
          <div className="space-y-6">
            <form onSubmit={handleSearch} className="flex gap-2">
              ...
              <button
                ...
                className="px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:bg-gray-400 active:bg-blue-800 cursor-pointer"
              >
                {isSearching ? '搜尋中...' : '🔍 搜尋歷史'}
              </button>
            </form>
                  ...
                  <div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg mb-4 text-black">
                      <h3 className="text-lg font-bold text-blue-900">
                        車牌號碼：{searchResult.vehicle.plate_number}
                      </h3>
                      <p className="text-sm text-gray-700">車型：{searchResult.vehicle.model || '未設定'}</p>
                      <p className="text-sm text-gray-700">最新記錄里程：{searchResult.vehicle.mileage} km</p>
                    </div>

                    {/* 工單歷史清單 */}
                    <h4 className="font-bold text-gray-800 mb-3">過往工單紀錄：</h4>
                    <div className="space-y-4">
                      {searchResult.workOrders?.map((wo: any) => (
                        <div key={wo.id} className="border rounded-lg p-4 bg-gray-50 text-black">
                          ...
                          <p className="text-sm text-gray-600 mb-2">描述：{wo.description || '無'}</p>

                          <div className="bg-white p-2 rounded border text-sm">
                            <div className="font-semibold text-gray-700 mb-1">維修細項：</div>
                            <ul className="list-disc list-inside space-y-1">
                              {wo.work_order_items?.map((item: any) => (
                                <li key={item.id} className="text-gray-600">
                                  {item.item_name} ({item.type === 'Part' ? '零件' : '工時'}) - 數量: {item.quantity} x ${item.unit_price} = ${item.subtotal}
                                </li>
                              ))}
                            </ul>
                          </div>
